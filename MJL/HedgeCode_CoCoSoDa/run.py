...
import numpy as np
from HCLModel import HCLModel
from MJLModel import MJLModel
from itertools import cycle
from model import Model, Multi_Loss_CoCoSoDa
from torch.nn import CrossEntropyLoss
from torch.utils.data import DataLoader, Dataset, SequentialSampler, RandomSampler, SubsetRandomSampler
...
javascript_special_token = ['keyword', 'separators', 'identifier', 'property_identifier', 'operator',
                            'number', 'string_fragment', 'comment', 'regex_pattern',
...
def convert_examples_to_features_aug_type(js):
    ...
    code_type_tokens = [tokenizer.cls_token, "<encoder-only>", tokenizer.sep_token] + code_type_tokens + [
        tokenizer.sep_token]
...
def mask_tokens(inputs, tokenizer, mlm_probability):
    ...
    indices_random = torch.bernoulli(torch.full(labels.shape, 0.5)).bool().to(
...
def multi_lang_continue_pre_train(args, model, tokenizer, pool):
    ...
    train_dataloaders = [
        cycle(DataLoader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size, drop_last=True)) for
        train_dataset, train_sampler in zip(train_datasets, train_samplers)]
    ...
    while True:
        ...
        if step % 4 == 0:
            code_transformations_ids[:, 3:], _ = mask_tokens(code_inputs.clone()[:, 3:], tokenizer,
            ...
            code_transformations_ids[:, 3:], _ = replace_with_type_tokens(code_inputs.clone()[:, 3:],
                                                                          code_types.clone()[:, 3:], tokenizer,
                                                                          args.mlm_probability)
        elif step % 4 == 2:
            random.seed(step)
            choice_token_id = choice(special_token_id_list)
            code_transformations_ids[:, 3:], _ = replace_special_token_with_type_tokens(code_inputs.clone()[:, 3:],
                                                                                        choice_token_id, tokenizer,
                                                                                        args.mlm_probability)
        elif step % 4 == 3:
            random.seed(step)
            choice_token_id = choice(special_token_id_list)
            code_transformations_ids[:, 3:], _ = replace_special_token_with_mask(code_inputs.clone()[:, 3:],
                                                                                 choice_token_id, tokenizer,
                                                                                 args.mlm_probability)
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if global_step % 100 == 0:
                logger.info(" global steps (step*gradient_accumulation_steps ): %s loss: %s", global_step,
                            round(avg_loss, 6))
            ...
            if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                ...
                if not os.path.exists(output_dir):
                    os.makedirs(output_dir)
                model_to_save = model.module.code_encoder_q if hasattr(model,
                                                                       'module') else model.code_encoder_q  # Take care of distributed/parallel training
    ...
    for batch in query_dataloader:
        ...
        with torch.no_grad():
            if args.model_type == "base":
                ...
                if args.agg_way == "avg":
                    outputs = outputs[0]
                    nl_vec = (outputs * nl_inputs.ne(1)[:, :, None]).sum(1) / nl_inputs.ne(1).sum(-1)[:,
                                                                              None]  # None作为ndarray或tensor的索引作用是增加维度，
                elif args.agg_way == "cls_pooler":
                    nl_vec = outputs[1]
                elif args.agg_way == "avg_cls_pooler":
                    nl_vec = outputs[1] + (outputs[0] * nl_inputs.ne(1)[:, :, None]).sum(1) / nl_inputs.ne(1).sum(-1)[:,
                                                                                              None]
    ...
    for batch in code_dataloader:
        with torch.no_grad():
            ...
            if args.model_type == "base":
                ...
                if args.agg_way == "avg":
                    outputs = outputs[0]
                    code_vec = (outputs * code_inputs.ne(1)[:, :, None]).sum(1) / code_inputs.ne(1).sum(-1)[:,
                                                                                  None]  # None作为ndarray或tensor的索引作用是增加维度，
                elif args.agg_way == "cls_pooler":
                    code_vec = outputs[1]
                elif args.agg_way == "avg_cls_pooler":
                    code_vec = outputs[1] + (outputs[0] * code_inputs.ne(1)[:, :, None]).sum(1) / code_inputs.ne(1).sum(
                        -1)[:, None]
...
def main():
    ...
    logger.info("config_name: %s", args.config_name)
    logger.info("model_name_or_path: %s", args.model_name_or_path)
    logger.info("tokenizer_name: %s", args.tokenizer_name)
    ...
    model_base = create_model(args, model, tokenizer, config)
